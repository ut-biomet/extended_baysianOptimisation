...
breedSimOpt3 <- function(i,
  ...
  newParams <- list(i = i,
                    ...
                    budget = budget,
                    nGen = nGen,
                    currentSelectionCycle = currentSelectionCycle,
                    nIndIni = currentPop$nInd,
                    plotCost = plotCost,
                    newIndCost = newIndCost)
  params <- do.call(getSimulParams3, newParams)

  if (verbose) {
    print(params)
  }

  finalGVs <- simuleBreeding3(nPheno = params$nPheno,
                              nSelected = params$nSelected,
                              nNew = params$nNew,
                              nGen = nGen - (currentSelectionCycle-1),
...
simuleBreeding3 <- function(nPheno,
                            nSelected,
                            nNew,
                            nGen,
                            currentPop,
  ...
  for (gen in seq(nGen)) {
    ...
    if (nPheno[gen] != 0) {
      ...
      genoDta <- rbind(genoDta, newGenoData)
      model <- createModel(phenoDta, genoDta, trait$name)
    }
...
getSimulParams3 <- function(i,
  ...
  # initialisation
  nSelected <- rep(0, nGen - (currentSelectionCycle - 1))

  # 1 - calculation of the total number of new individuals and phenotyping plots
  phenoGen <- seq(1, nGen, phenoFreq)
  remainingPhenoGen <- phenoGen[phenoGen >= currentSelectionCycle]
  x <- calcNTotnPheno3(bRep = bRep,
                       budget = budget,
                       plotCost = plotCost,
                       newIndCost = newIndCost,
                       minInds = nGen - (currentSelectionCycle - 1),
                       remainingPhenoGen = remainingPhenoGen)
  ...
  nSelected <- pmax(round(c(nIndIni, nNew[-length(nNew)]) * i), 1)

  # 3 - calc number of phenotyping per generation
  nPheno <- calcNpheno3(phenoFreq = phenoFreq,
                        nPhenoTot = nPhenoTot,
                        nGen = nGen,
                        currentSelectionCycle = currentSelectionCycle,
                        remainingPhenoGen = remainingPhenoGen)
  ...
  # checks
  nInd <- c(nIndIni, nNew[1:(nGen - (currentSelectionCycle - 1)) - 1])
...
calcNTotnPheno3 <- function(bRep, budget, plotCost = 1, newIndCost = 1.07, minPheno = 3, minInds = 5, remainingPhenoGen = c()){


  if (length(remainingPhenoGen) == 0) {
    warning(paste("No more phenotyping will happend. All budget will go",
                  "in new individuals creation whatever `bRep`.\n"))
    indTot <- max(round(budget / newIndCost), 1)
    nPheno <- 0
    remain <- budget - nPheno*plotCost - indTot*newIndCost
    return(c(indTot = indTot,
             nPheno = nPheno,
             remain = remain))
  }
...
calcNpheno3 <- function(phenoFreq,
                        nPhenoTot,
                        nGen,
                        remainingPhenoGen,
                        currentSelectionCycle){

  nPheno <- rep(0, nGen - (currentSelectionCycle - 1))
  if (length(remainingPhenoGen) == 0) {
    return(nPheno)
  }

  nPheno[remainingPhenoGen - (currentSelectionCycle-1)] <- floor(nPhenoTot / length(remainingPhenoGen))
  ...
  if (sum(nPheno) != nPhenoTot) {
    ...
    phenoGen <- sample(remainingPhenoGen - (currentSelectionCycle-1), remain, replace = F)
